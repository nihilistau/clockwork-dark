...
The screens that are THIS story's, as one Blueprint.
...
journal to satisfy a client that assumes one; a story that trades in favours
rather than coin should not be handed a vendor table. So they live behind a
blueprint a second story simply does not mount -- it names its own in its
...
and would in fact work unchanged for any story that ships those files. They sit
here because a story must be able to DELETE them, not because they could not be
shared. ``/api/quests``, the codex and ``/api/trade`` are the ones that reach
for Clockwork-shaped content.
...
BLUEPRINT_NAME = "clockwork_story"
...
def _load_item_registry() -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(root.glob("*.yaml")):
        try:
            ...
            logger.warning("[clockwork_api] Item file unreadable (%s): %s", path, exc)
            continue
...
def _load_recipe_registry() -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(root.glob("*.yaml")):
        try:
            ...
            logger.warning("[clockwork_api] Recipe file unreadable (%s): %s", path, exc)
            continue
...
def story_blueprint(store: SessionStore, name: str = BLUEPRINT_NAME) -> Blueprint:
    """
    Build the Clockwork story blueprint.
