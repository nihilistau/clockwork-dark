...
| `noticed: true` on a failed `lift_purse` | `engine/world/thievery.py`, effect on `GameState` | A mark cannot be re-offered the same in-game day (a `lifted_<npc_id>_d<day>` flag through `apply_effect`, final-review fix), but a **noticed** lift still has no consequence beyond that turn's prose -- no witness fact, no wanted-band movement. Reader: the Law (v0.10.0), whose witness/report pass is what `noticed` was written for. |
| Hot-goods reporting by an honest vendor | `engine/game/trade.py::_plan_sale` / `trade.sell` | An honest vendor (no `fence: true`) refuses a hot sale in its own voice but never reports the attempt, though the `fencing` deed exists in the Law's design (§2) for exactly this. Reader: the Law (v0.10.0). |
| Evaluator vs. a clean getaway narrated over `noticed: true` | `engine/agents/evaluator.py` (the outcome-aware evaluator from v0.8) | `lift_purse` is in `ROLLING_SKILLS` and the evaluator catches success-over-failure on the roll itself, but nothing checks prose that narrates a clean, unseen getaway over a receipt carrying `noticed: true` -- a second axis the v0.8 contradiction check does not cover. Reader: the Law (v0.10.0), once being noticed has a consequence worth protecting in prose. |
