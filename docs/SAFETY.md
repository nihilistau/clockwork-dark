...
| **The player boundary sheet** | `engine/safety/policy.py::set_policy` — no production caller (only a re-export in `engine/safety/__init__.py`). `resolve(player=...)` is never called with boundaries | **This document says elsewhere that limits are "set in the boundary sheet at the start of a run", and `games/wicked-garden/game.yaml` says the same. There is no boundary sheet.** No surface anywhere collects one, so `hard_nos`, `soft_nos` and `green_lights` are permanently empty and `engine/safety/boundaries.py` + `engine/safety/redirect.py` cannot fire in a real session. What DOES work is the story ceiling and the intensity dial — the parts driven by config and the Settings screen. Needs a session-setup surface that calls `set_policy`. Recorded 2026-08-15 |
| **The Fade control** | `engine/safety/policy.py::fade_available` — computed from three layers, read by nothing (`grep -rn fade_available` outside `policy.py` → 0 hits) | The config table below lists `safety.fade.available` as "offer the Fade control". **No such control exists**: there is no button, and nothing consults the flag. The fade CARD is real and wired (`ui/src/core/parts/FadeCard.jsx`) — that is the engine deciding to fade a scene, not the player asking it to. Needs a control in `Play.jsx` that submits a fade request. Recorded 2026-08-15 |
